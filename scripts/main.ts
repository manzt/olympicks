	...
	.object({
		...
		match: z.union([
			z
				...
				.transform((data) => ({
					team1: {
						description: data.team1.description,
						code: data.team1.teamCode,
					},
					team2: {
						description: data.team2.description,
						code: data.team2.teamCode,
					},
				})),
			z.object({}),
			z.undefined(),
...
let nextDataSchema = z
	...
	.transform((data) => {
		let item = data.props.pageProps.page.items.find(
			(item) => item.name === "scheduleWrapper",
		);
