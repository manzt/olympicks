	...
	for (let event of group) {
		...
		if (!current) {
			...
			if (currentEnd >= eventStart) { // overlapping
				...
				current.end = new Date(Math.max(currentEnd, new Date(event.end))).toISOString();
...
function combineEntries(events) {
	...
	let matches = events
		.filter(e => e.match)
		.map(e => ({
			title: `olympicks: ${e.discipline.description}${formatMatchEntry(e.match)}${formatMedalEvent(e.medal)}`,
			start: e.start,
			end: e.end,
...
export function createIcsEntry(events) {
	let result = ics.createEvents(combineEntries(events).map((event) => ({
		title: event.title,
